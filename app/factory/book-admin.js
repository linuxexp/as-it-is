export class Chapter {
    constructor(index, title, purport, verses) {
        this.entity = 'chapter';
        this.index = index;
        this.title = title || "";
...
angular.module("angular-common")
    .factory("bookAdmin", function () {

        // store book in this, update/delete in memory
        // perform immutable updates
        let book = [];

        const cloneBook = (book) => _.cloneDeep(book);
        const immutableChange = (updateFunc) => {
            const bk = cloneBook(book);
            book = updateFunc(bk);
            return book;
        };
        return {
            addChapter: (chapter) => {
                return immutableChange((book) => {
                    book.push(chapter);
                    return book;
                });
            },
            ...
            addVerse: (chapterIndex) => null,
            updateVerse: (chapterIndex, verseIndex, verse) => null,
            getBook: () => book,
            saveBook: () => null,
        }
    });